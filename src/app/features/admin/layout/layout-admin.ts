...
export class AdminLayoutComponent implements OnInit {
    ...
    private loadLayout(): void {
        this.adminService.loadDefaultLayout().subscribe({
    ...
    goToNextStep(): void {
        ...
        if (current === 'channels' && this.canProceedFromChannels()) this.currentStep.set('order');
        else if (current === 'order' && this.canProceedFromOrder()) this.currentStep.set('categories');
        this.cancelSwap();
    }

    goToPreviousStep(): void {
        const current = this.currentStep();
        if (current === 'categories') this.currentStep.set('order');
        else if (current === 'order') this.currentStep.set('channels');
        else if (current === 'channels') this.currentStep.set('home');
        this.cancelSwap();
    }
    ...
    selectAllChannels(): void { this.channelSelections.forEach(cs => cs.selected = true); }
    deselectAllChannels(): void { this.channelSelections.forEach(cs => cs.selected = false); }
    ...
    handleItemTap(index: number): void {
        if (this.swapSourceIndex === null) { this.swapSourceIndex = index; return; }
        if (this.swapSourceIndex === index) { this.swapSourceIndex = null; return; }
        const a = this.swapSourceIndex;
        const b = index;
        this.animateReorder(() => {
            const sel = this.getSelectedChannels();
            const A = sel[a];
            const B = sel[b];
    ...
    cancelSwap(): void { this.swapSourceIndex = null; }
    ...
    private movePosition(fromIdx: number, toIdx: number): void {
        const sel = this.getSelectedChannels();
        const dragged = sel[fromIdx];
        const target = sel[toIdx];
        const draggedPos = dragged.position;
        const targetPos = target.position;

        if (draggedPos < targetPos) {
            this.channelSelections.forEach(ch => {
                if (ch.selected && ch.position > draggedPos && ch.position <= targetPos) ch.position--;
            });
        } else {
            this.channelSelections.forEach(ch => {
                if (ch.selected && ch.position >= targetPos && ch.position < draggedPos) ch.position++;
            });
        }
        dragged.position = targetPos;
    }
    ...
    saveLayout(): void {
        ...
        selectedChannels.forEach((ch, idx) => ch.position = idx);

        this.adminService.saveDefaultLayout(this.channelSelections).subscribe({
            next: () => {
                this.saved.set(true);
    ...
    // ===== Helpers =====
    canProceedFromChannels(): boolean { return this.getSelectedCount() > 0; }
    canProceedFromOrder(): boolean    { return this.channelSelections.length > 0; }
    canSaveLayout(): boolean          { return this.channelSelections.length > 0; }
}
